...
선별 원칙:
1. 완전히 동일한 기사, 동일 링크, 재송고성 유사 기사만 제거하라.
2. 같은 이슈라도 매체 관점이나 내용 포인트가 다르면 별도 기사로 인정할 수 있다. (기사 내용 확인이 필요함)
3. 아래 기사는 우선 제외:
   - 순수 정치
   - 일반 사건사고
   - 단순 지역 행사
...
6. 단순하게 헤드라인을 읽지 말고 맥락을 파악해라.
7. 걸러지고 나서 매체력이 높은 걸 선별하라 (동일하다고 판단되는 기사가 다수 일때 적용, 이외에 다른 매체에서 밸류 있는 기사일 때 적용하는 게 아님)
   - Tier 1: 조선일보, 중앙일보, 동아일보, 매일경제, 한국경제, 연합뉴스, 뉴시스, 뉴스1
   - Tier 2: 서울경제, 머니투데이, 이데일리, 파이낸셜뉴스, 아시아경제, 전자신문, 아주경제
   - Tier 3: 디지털데일리, 디지털투데이, 아이뉴스24, 뉴스웨이, 오토타임즈, M투데이 등
